...
public class BinaryOp extends IRInstruction {
    ...
    public BinaryOp(BasicBlock basicBlock, BinaryOpName op, Operand lhs, Operand rhs, Register result) {
        super(basicBlock);
        this.op = op;
        this.lhs = lhs;
        this.rhs = rhs;
    ...
    private boolean mergeConstantAndSwapOperands(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        if (lhsPrivilege == 2 && rhsPrivilege == 2) {
            ...
            // u = x + y
            // r = u + (a + b)
            BinaryOp lhsDef = ((BinaryOp) ((Register) lhs).getDef());
            BinaryOp rhsDef = ((BinaryOp) ((Register) rhs).getDef());
            ...
            if (lhsDef.getRhs() instanceof Constant && rhsDef.getRhs() instanceof Constant) {
                ...
                BinaryOp newInst = new BinaryOp(this.getBasicBlock(), this.op,
    ...
    private boolean combineAdd(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        // z = x + (-y)  --->  z = x - y
        if (rhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) rhs).getDef()).isNegative()) {
            op = BinaryOpName.sub;
            replaceRhs(((BinaryOp) ((Register) rhs).getDef()).getNegativeOperand());
            pushResultInQueue(queue, inQueue);
            return true;
        }
        // z = (-x) + y  --->  z = y - x
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).isNegative()) {
            Operand newLhs = rhs;
            Operand newRhs = ((BinaryOp) ((Register) lhs).getDef()).getNegativeOperand();
            op = BinaryOpName.sub;
            replaceLhs(newLhs);
            replaceRhs(newRhs);
            pushResultInQueue(queue, inQueue);
        ...
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).op == BinaryOpName.sub) {
            Operand a = ((BinaryOp) ((Register) lhs).getDef()).lhs;
            Operand b = ((BinaryOp) ((Register) lhs).getDef()).rhs;
            // y = ((-c) - b) + c  --->  y = 0 - b
            if (a.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) a).getDef()).isNegative()
                    && rhs == ((BinaryOp) ((Register) a).getDef()).getNegativeOperand()) {
        ...
        if (rhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) rhs).getDef()).op == BinaryOpName.sub) {
            Operand b = ((BinaryOp) ((Register) rhs).getDef()).lhs;
            Operand c = ((BinaryOp) ((Register) rhs).getDef()).rhs;
            // y = a + ((-a) - c)  --->  y = 0 - c
            if (b.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) b).getDef()).isNegative()
                    && lhs == ((BinaryOp) ((Register) b).getDef()).getNegativeOperand()) {
                Operand newLhs = new ConstInt(IntegerType.BitWidth.int32, 0);
                op = BinaryOpName.sub;
                replaceLhs(newLhs);
                replaceLhs(c);
    ...
    private boolean combineSub(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        // z = (-x) - (-y)  --->  z = y - x
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).isNegative()
                && rhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) rhs).getDef()).isNegative()) {
            Operand newLhs = ((BinaryOp) ((Register) rhs).getDef()).getNegativeOperand();
            Operand newRhs = ((BinaryOp) ((Register) lhs).getDef()).getNegativeOperand();
            replaceLhs(newLhs);
            replaceRhs(newRhs);
            pushResultInQueue(queue, inQueue);
        ...
        if (this.isNegative() && this.getNegativeOperand().registerDefIsBinaryOpInst()
                && ((BinaryOp) ((Register) this.getNegativeOperand()).getDef()).op == BinaryOpName.sub) {
            Operand newLhs = ((BinaryOp) ((Register) this.getNegativeOperand()).getDef()).rhs;
            Operand newRhs = ((BinaryOp) ((Register) this.getNegativeOperand()).getDef()).lhs;
            replaceLhs(newLhs);
            replaceRhs(newRhs);
            pushResultInQueue(queue, inQueue);
            return true;
        ...
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).op == BinaryOpName.add) {
            Operand a = ((BinaryOp) ((Register) lhs).getDef()).lhs;
            Operand b = ((BinaryOp) ((Register) lhs).getDef()).rhs;
            // y = (c + b) - c  --->  b replace y
            if (a == rhs) {
        ...
        // y = a - (b + c): check a - b, a - c
        if (rhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) rhs).getDef()).op == BinaryOpName.add) {
            Operand b = ((BinaryOp) ((Register) rhs).getDef()).lhs;
            Operand c = ((BinaryOp) ((Register) rhs).getDef()).rhs;
            // y = a - (a + c)  --->  y = 0 - c
            if (lhs == b) {
    ...
    private boolean combineMul(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        if (power > 0) {
            ...
            BinaryOp shiftLeft = new BinaryOp(this.getBasicBlock(),
    ...
    private boolean combineShl(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        // y = (x << a) << b  --->  y = x << (a + b)
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).op == BinaryOpName.shl) {
            Operand a = ((BinaryOp) ((Register) lhs).getDef()).rhs;
            if (a instanceof ConstInt && rhs instanceof ConstInt) {
                Operand newLhs = ((BinaryOp) ((Register) lhs).getDef()).lhs;
        ...
        // y = (x * a) << b  --->  y = x * (a << b)
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).op == BinaryOpName.mul) {
            Operand a = ((BinaryOp) ((Register) lhs).getDef()).rhs;
            if (a instanceof ConstInt && rhs instanceof ConstInt) {
                Operand newLhs = ((BinaryOp) ((Register) lhs).getDef()).lhs;
    ...
    private boolean combineAshr(Queue<IRInstruction> queue, Set<IRInstruction> inQueue) {
        ...
        // y = (x >> a) >> b  --->  y = x >> (a + b)
        if (lhs.registerDefIsBinaryOpInst() && ((BinaryOp) ((Register) lhs).getDef()).op == BinaryOpName.ashr) {
            Operand a = ((BinaryOp) ((Register) lhs).getDef()).rhs;
            if (a instanceof ConstInt && rhs instanceof ConstInt) {
                Operand newLhs = ((BinaryOp) ((Register) lhs).getDef()).lhs;
                Operand newRhs = calculateConstant(BinaryOpName.add, ((ConstInt) a), ((ConstInt) rhs));
    ...
    @Override
    public Object clone() {
        BinaryOp binaryOpInst = (BinaryOp) super.clone();
